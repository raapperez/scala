 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a number") {
    assert(sum(List(42)) === 42)
  }

  test("sum of lot of numbers") {
    assert(sum(List(42, 200, 57, 83, 23, -123, -1999, 2338498)) === 2336781)
  }
  ...
  test("max of a few negative number") {
    assert(max(List(-4, -6, -12, -40)) === -4)
  }

  test("max of a empty list throws NoSuchElementException") {
